...
`injectPresent(sections, activeVar, shouldResetActive)` 会查找原文件中的 `[Present]`。
...
勾选「修复跨角色面板」时，会检查 `[Present]` 中是否已有 `post $active = 0`（变量名使用当前 `activeVar`）。没有就补一行；如果原文件没有 `[Present]`，新建的 `[Present]` 末尾也会追加这行。

### 9.3 Beta 多 active 面板

勾选 `[Beta] 多形态角色支持` 时，`buildIni()` 会改走 `buildBetaIni()`，普通模式不受影响。

Beta 模式会扫描所有包含 `active` 字眼的变量，例如 `$active0`、`$black_active`、`$active`。用户也可以在 UI 列表里手动增删。生成时按每个 active 变量筛选 `condition` 中包含该变量的 `[Key*]`，为每组生成独立的 `CommandListGuiMenuBeta*`、`CommandListGuiClickBeta*` 和 `res_gui/beta_*` 资源。

面板标题图片使用 `<MOD名>-<active变量名>`，例如 `MyMod-black_active`。只有对应 active 条件成立时，才会渲染对应面板。
...
20. 勾选「修复跨角色面板」后，生成 ini 的 `[Present]` 会补 `post $active = 0`，且不会重复添加
21. 勾选 `[Beta] 多形态角色支持` 后，能扫描 `$active` / `$black_active` 等变量，并按变量生成独立面板资源
22. 右键「绑定按键」弹出 prompt，修改后预览和 INI 同步
23. 面板预览和 INI 预览并排正常渲染
24. FAB + 号按钮展开/收起子菜单正常，子菜单 3 个按钮可点击
25. 进入批量选择模式后 slot 显示复选框，确认/取消按钮正常工作
26. 批量合并 2+ 按钮后，合并按钮 groups > 1，"拆分"菜单项可见
27. 右键拆分合并按钮后，恢复为独立按钮，groups 各为 1
28. 面板拖拽排序后 skip 按钮保持原位，活跃按钮顺序可保存
29. 面板背景 PNG 分辨率随 panelW × panelH 变化，不再固定 256x256
30. 调色面板能进入/退出，预设配色和恢复默认能即时刷新预览
31. 禁用标题文字投影后，网页预览和导出 `title.png` 都不绘制投影
32. 上传按钮图和面板图后，刷新并恢复草稿，导出 ZIP 仍包含对应图片效果
33. Chrome / Edge 下选择 MOD 根目录后，能递归列出子目录里的 ini，并忽略 `DISABLED*` 文件/文件夹
34. 点击写入文件夹后，在选中 ini 所在文件夹生成 ini 和 `res_gui/*`，默认写回原 `.ini` 文件名，并把原内容备份成 `.txt`
