...
21. 右键「绑定按键」弹出 prompt，修改后预览和 INI 同步
22. 面板预览和 INI 预览并排正常渲染
23. FAB + 号按钮展开/收起子菜单正常，子菜单 3 个按钮可点击
24. 进入批量选择模式后 slot 显示复选框，确认/取消按钮正常工作
25. 批量合并 2+ 按钮后，合并按钮 groups > 1，"拆分"菜单项可见
26. 右键拆分合并按钮后，恢复为独立按钮，groups 各为 1
27. 面板拖拽排序后 skip 按钮保持原位，活跃按钮顺序可保存
28. 面板背景 PNG 分辨率随 panelW × panelH 变化，不再固定 256x256
29. 调色面板能进入/退出，预设配色和恢复默认能即时刷新预览
30. 禁用标题文字投影后，网页预览和导出 `title.png` 都不绘制投影
31. 上传按钮图和面板图后，刷新并恢复草稿，导出 ZIP 仍包含对应图片效果
32. Chrome / Edge 下选择 MOD 根目录后，能递归列出子目录里的 ini，并忽略 `DISABLED*` 文件/文件夹
33. 点击写入文件夹后，在选中 ini 所在文件夹生成 ini 和 `res_gui/*`，默认写回原 `.ini` 文件名，并把原内容备份成 `.txt`
