...
如果使用 Chrome / Edge，也可以点"选择 MOD 文件夹"，工具会递归列出该目录和子目录里的 `.ini`；勾选旁边的"包含 txt 文件"后也会列出 `.txt`，切换后会立即刷新列表。选中要处理的文件后可直接写回它所在的文件夹；工具会生成 `.gui.ini` 和 `res_gui`，并默认把原 `.ini` 改成 `.txt`。扫描时会忽略任何名称以 `DISABLED` 开头的文件和文件夹（不区分大小写）。

文件夹模式会直接写入文件夹并改名原 ini，请确认自己知道在处理哪个 MOD。不要选择包含大量 ini 的大目录，否则扫描和下拉列表可能会很卡。
