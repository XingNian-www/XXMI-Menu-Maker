...
如果使用 Chrome / Edge，也可以点"选择 MOD 文件夹"，工具会递归列出该目录和子目录里的 `.ini`；勾选旁边的"包含 txt 文件"后也会列出 `.txt`，切换后会立即刷新列表。选中要处理的文件后可直接写回它所在的文件夹；工具会生成 ini 和 `res_gui`，默认使用原 `.ini` 文件名，并把原 `.ini` 内容备份成 `.txt`。处理 `.txt` 时原文件不动，生成文件使用 `_gui.ini` 后缀。扫描时会忽略任何名称以 `DISABLED` 开头的文件和文件夹（不区分大小写）。
