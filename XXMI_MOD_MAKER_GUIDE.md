...
## XXMI Menu Maker 使用补充

本项目的 `index.html` 是一个单页 GUI 生成工具，用于把已有 `[Key*]` 配置转换为可点击的游戏内 GUI 面板。下面是当前工具层面的使用要点，和后面的 XXMI / 3DMigoto 命令说明相互独立。

### 匹配图标

- 右键按钮后使用 `匹配图标`，入口会直接打开图标库搜索，不再保留单独的自动匹配入口
- 图标库混合显示 Lucide 和 Iconify，默认进入 `混合` 模式；切换按钮顺序是 `混合 / 分组`
- `分组` 模式按图标包分组，并优先显示已缓存的 Iconify 图标包
- 图标结果会显示图标包名和图标名，便于区分同名图标
- 搜索关键词会去掉末尾数字，例如 `Top1`、`Top2` 会按 `Top` 匹配并合并候选词
- Lucide 是内置图标包，不需要下载，也不能删除缓存；Iconify 可以预下载、删除缓存、顶置图标包
- 图标包示例优先使用 Iconify collections 元数据里的 sample；没有可用示例时会显示图标包前缀占位

### 合并按钮

- 同 Key 按钮会直接合并；大小写不同也视为同 Key
- Key 比较会忽略正确的 `no_modifiers` 前缀，所以 `8` 和 `no_modifiers 8` 会按同一个 Key 分组
- 工具不会纠正拼写错误，例如 `no_modifers` 不会被当成 `no_modifiers`
- 不同 Key 合并时会弹出二次确认：
  - `所有 Key 等效于按一次 GUI 按钮`：每个原 Key 都会执行合并后的全部逻辑
  - `Key 保持原逻辑，仅 GUI 点一下执行全部`：键盘按键仍保持原逻辑，只有 GUI 按钮执行全部逻辑
- 合并后的按钮会尝试继承选中项里优先级最高的非上传图标

### 文件夹写入

- 处理 `.ini` 时会先把原内容备份成 `.txt`，再写入新 ini 和 `res_gui`
- 如果关闭 `使用原 ini 名`，也会等新文件和资源全部写入成功后，才删除原 `.ini`
- 处理 `.txt` 时不会备份，因为原 `.txt` 不会被修改
- 调色里的面板边框和按钮边框预览会和导出资源保持一致
